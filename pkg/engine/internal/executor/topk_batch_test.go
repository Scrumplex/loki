...
func Test_iterContiguousRanges(t *testing.T) {
	tt := []struct {
		...
		{
			name:   "two separate contiguous ranges",
			rows:   []int{0, 1, 2, 7, 8, 9},
			ranges: []struct{ start, end int }{{0, 3}, {7, 10}},
		},
	}

	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			var actualRanges []struct{ start, end int }
			iterContiguousRanges(tc.rows, func(start, end int) bool {
				actualRanges = append(actualRanges, struct{ start, end int }{start, end})
				return true
			})
			require.Equal(t, tc.ranges, actualRanges, "ranges should match")
		})
	}

	t.Run("early exit", func(t *testing.T) {
		rows := []int{1, 2, 3, 5, 6, 7, 9, 10}
		var yieldedRanges []struct{ start, end int }
		iterContiguousRanges(rows, func(start, end int) bool {
			yieldedRanges = append(yieldedRanges, struct{ start, end int }{start, end})
			// Stop after first range
			return len(yieldedRanges) < 1
		})
		require.Equal(t, []struct{ start, end int }{{1, 4}}, yieldedRanges, "should only yield first range")
	})
}
